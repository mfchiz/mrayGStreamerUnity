...
// this listener is used for pre-decoding stage, when we are still receiving the
// rtp packets for the frame
class VideoRTPDataListener : public IMyListenerCallback {
 ...
 public:
  ...
  void ListenerOnDataChained(_GstMyListener* src, GstBuffer* buffer) {
    if (src == _pre) PreRTP(buffer);
    if (src == _post) PostRTP(buffer);
  }
  ...
  void PreRTP(GstBuffer* buffer) {
    ...
    if (true) {
      // check if the timestamp of this packet is different from the one
      // on top of our timestamp queue. if so, then add the packet
      ...
      if (okay) {
        ...
        packet.length = rtp_padding_payload((unsigned char*)map.data, map.size,
                                            packet.data);
  ...
  RTPPacketData GetLastRTPPacket(bool remove) {
    ...
    if (timestampQueue.size() == 0) return RTPPacketData();
    //  			while (timestampQueue.size() > maxPackets)
    //  				timestampQueue.pop_front();
    RTPPacketData ts = timestampQueue.front();
    if (remove) timestampQueue.pop_front();
    return ts;
  }
  void AddRTPPacket(const RTPPacketData& ts) {
    ...
    if (timestampQueue.size() == 0) {
      ...
      const RTPPacketData& last = timestampQueue.back();
      if (last.timestamp != ts.timestamp) timestampQueue.push_back(ts);
    }
  }
};
...
class VideoPreappDataListener : public IMyListenerCallback {
 ...
 public:
  ...
  RTPPacketData GetLastRTPPacket(ulong bufferID, bool remove) {
    std::lock_guard<std::recursive_mutex> lock(m_mutex);
    if (m_packets.size() == 0) return RTPPacketData();
    PacketData packet;
    do {
...
void VideoAppSinkHandler::SetRTPListener(GstMyListener* preRTP,
                                         GstMyListener* postRTP,
                                         GstMyListener* preapp) {
...
GstFlowReturn VideoAppSinkHandler::process_sample(
  ...
  if (m_pixels[0].data.imageData && dataSize != (int)size) {
    if (vinfo.stride[0] < (m_pixels[0].data.Size.x * pxSize)) {
      ...
      std::stringstream ss;
      ss << "VideoAppSinkHandler::process_sample(): error on new buffer, "
            "buffer size: "
         << size << "!= init size: " << dataSize;
      LogMessage(ss.str(), ELL_WARNING);
      return GST_FLOW_ERROR;
    }
  }
  // printf("Received %dx%d\n", vinfo.width, vinfo.height);

  buffer = sample;

  if (!m_pixels[0].data.imageData) {
    //	m_data->rtplistener->GetLastRTPPacket(true);//remove one packet
    //	m_backPixels[0].RTPPacket =
    // m_data->preapplistener->GetLastRTPPacket((ulong)_buffer, true);
    if (RGBFormat)
  ...
  if (m_pixels[0].data.imageData) {
    ...
    /*	if (vinfo.stride[0] == m_pixels[0].data.Size.x*pxSize)
            {
                    m_backPixels[0].data.setData(mapinfo.data,
       m_pixels[0].data.Size, m_pixels[0].data.format);
            }
            else*/
    {
    ...
    // 		}
    // 		else {
    // 			m_backPixels[0].setData(mapinfo.data,
    // m_pixels[0].Size, m_pixels[0].format);
    // 			m_eventPixels.setData(mapinfo.data,
    // m_pixels[0].Size, m_pixels[0].format);
    // 		}
...
float VideoAppSinkHandler::GetCaptureFrameRate() { return m_frameCount; }
